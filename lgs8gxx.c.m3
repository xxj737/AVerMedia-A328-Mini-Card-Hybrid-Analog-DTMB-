...
static int lgs8g75_init_data(struct lgs8gxx_state *priv)
{
	...
	/* ===== A328-PROBE: post-download diagnostics =====
	 * (1) I2C write/read-back sanity on reg 0x0C
	 * (2) download datapath check via 0x3A/0x3B/0x3C read-back
	 * (3) register map after download
	 * (4) issue the 8051 start sequence (from set_mode_manual GI595
	 *     branch -- the ONLY place in the whole driver that ever writes
	 *     0x39=0x02/0x03) and see whether registers change
	 */
	{
		u8 v, v2, v3;
		int k;
		const u8 probe_regs[] = {0x00, 0x02, 0x0C, 0x13, 0x18, 0x19,
					 0x1F, 0x34, 0x39, 0x3D, 0xB0, 0xB1};

		/* (0) 内核态全地址扫描: 找出总线上真正会应答的设备。
		 * 只有返回非 0xFF 的地址才是真设备（0xFF = 总线浮空 = 无人应答）。
		 * TDA18271 @0x60 是阳性对照：它必须出现，否则扫描方法本身不可信。 */
		{
			int a;
			for (a = 0x08; a <= 0x77; a++) {
				u8 b0[] = { 0x00 };
				...
				int e1 = i2c_transfer(priv->i2c, mw, 2);
				int e2 = i2c_transfer(priv->i2c, mr, 1);
				dprintk("SCAN 0x%02X: reg00=0x%02X(%d) raw=0x%02X(%d)%s\n",
					a, b1[0], e1, r1[0], e2,
					(b1[0] != 0xFF || r1[0] != 0xFF) ?
						"   <<< RESPONDS" : "");
			}
		...
		/* (1) I2C 通路自检 */
		lgs8gxx_read_reg(priv, 0x0C, &v);
		dprintk("PROBE: 0x0C orig  = 0x%02X\n", v);
		lgs8gxx_write_reg(priv, 0x0C, 0x55);
		lgs8gxx_read_reg(priv, 0x0C, &v2);
		dprintk("PROBE: 0x0C<-0x55 = 0x%02X %s\n", v2,
			(v2 == 0x55) ? "OK" : "**FAIL**");
		lgs8gxx_write_reg(priv, 0x0C, 0xAA);
		lgs8gxx_read_reg(priv, 0x0C, &v3);
		dprintk("PROBE: 0x0C<-0xAA = 0x%02X %s\n", v3,
			(v3 == 0xAA) ? "OK" : "**FAIL**");
		lgs8gxx_write_reg(priv, 0x0C, v);	/* restore */

		/* (2) 下载数据通路自检 */
		lgs8gxx_write_reg(priv, 0x38, 0x00);
		lgs8gxx_write_reg(priv, 0x3A, 0x10);
		lgs8gxx_write_reg(priv, 0x3B, 0x00);
		lgs8gxx_write_reg(priv, 0x3C, 0x5A);
		lgs8gxx_read_reg(priv, 0x3A, &v2);
		dprintk("PROBE: 0x3A readback = 0x%02X (expect 0x10)\n", v2);
		lgs8gxx_read_reg(priv, 0x3B, &v2);
		dprintk("PROBE: 0x3B readback = 0x%02X (expect 0x00)\n", v2);
		lgs8gxx_read_reg(priv, 0x3C, &v2);
		dprintk("PROBE: 0x3C readback = 0x%02X (expect 0x5A)\n", v2);

		/* (3) 下载后寄存器地图 */
		for (k = 0; k < (int)sizeof(probe_regs); k++) {
			lgs8gxx_read_reg(priv, probe_regs[k], &v);
			dprintk("PROBE-MAP   reg%02X = 0x%02X\n",
				probe_regs[k], v);
		}
		...
		for (k = 0; k < (int)sizeof(probe_regs); k++) {
			lgs8gxx_read_reg(priv, probe_regs[k], &v);
			dprintk("PROBE-START reg%02X = 0x%02X\n",
				probe_regs[k], v);
		}

		/* (5) 再叠一次 soft_reset (mainline 在模式切换后都做) */
		lgs8gxx_soft_reset(priv);
		msleep(150);
		for (k = 0; k < (int)sizeof(probe_regs); k++) {
			lgs8gxx_read_reg(priv, probe_regs[k], &v);
			dprintk("PROBE-RESET reg%02X = 0x%02X\n",
				probe_regs[k], v);
		}
	}
