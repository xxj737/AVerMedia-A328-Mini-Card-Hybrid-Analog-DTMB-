...
/* ===== A328 诊断辅助函数（仅用于定位，最终版会删除） ===== */
static void lgs8gxx_probe_dump(struct lgs8gxx_state *priv, const char *tag)
{
	static const u8 r[] = {0x00, 0x02, 0x0C, 0x13, 0x18, 0x19, 0x1F,
			       0x34, 0x39, 0x3A, 0x3B, 0x3C, 0x3D, 0x7C,
			       0xB0, 0xB1, 0xC0, 0xC1};
	u8 v;
	int k;

	for (k = 0; k < (int)sizeof(r); k++) {
		if (lgs8gxx_read_reg(priv, r[k], &v) == 0)
			dprintk("DUMP[%-14s] reg%02X = 0x%02X\n", tag, r[k], v);
		else
			dprintk("DUMP[%-14s] reg%02X = ERR\n", tag, r[k]);
	}
}

static void lgs8gxx_probe_rb(struct lgs8gxx_state *priv, const char *tag,
			     u8 reg, u8 val)
{
	u8 before = 0, after = 0;

	lgs8gxx_read_reg(priv, reg, &before);
	lgs8gxx_write_reg(priv, reg, val);
	lgs8gxx_read_reg(priv, reg, &after);
	dprintk("RB[%-14s] reg%02X: before=0x%02X --wrote 0x%02X--> "
		"after=0x%02X  %s\n", tag, reg, before, val, after,
		(after == val) ? "LATCHED-OK" : "**NO-LATCH**");
	lgs8gxx_write_reg(priv, reg, before);	/* restore */
}
...
static int lgs8g75_init_data(struct lgs8gxx_state *priv)
{
	...
	/* A328 阶段 A: 下载前的原始状态 —— 此刻芯片处于 reg00=0xD4 的
	 * "BSL 就绪"态，是唯一可能正常访问寄存器文件的时机 */
	lgs8gxx_probe_dump(priv, "A-INIT");
	lgs8gxx_probe_rb(priv, "A-INIT", 0x0C, 0x55);
	lgs8gxx_probe_rb(priv, "A-INIT", 0x18, 0x66);
	lgs8gxx_probe_rb(priv, "A-INIT", 0x19, 0x77);
	lgs8gxx_probe_dump(priv, "A-AFTER-RB");
	...
	/* A328 阶段 B: 进入下载模式后 */
	lgs8gxx_probe_dump(priv, "B-DLMODE");
	...
	/* A328 阶段 C: 固件下载完成（仍在下载模式） */
	lgs8gxx_probe_dump(priv, "C-AFTER-DL");

	/* A328 阶段 D: 执行 8051 启动序列（全驱动唯一写 0x39=0x02/0x03 的地方，
	 * 原本被 set_mode_manual 的 GI595 分支条件卡住，从未被执行过） */
	lgs8gxx_write_reg(priv, 0x6E, 0x05);
	lgs8gxx_write_reg(priv, 0x39, 0x02);
	lgs8gxx_write_reg(priv, 0x39, 0x03);
	lgs8gxx_write_reg(priv, 0x3D, 0x05);
	...
	lgs8gxx_write_reg(priv, 0x53, 0x80);
	msleep(150);
	lgs8gxx_probe_dump(priv, "D-AFTER-8051START");

	/* A328 阶段 E: soft_reset 退出下载模式后（回到 reg00=0xD4 态） */
	lgs8gxx_soft_reset(priv);
	msleep(150);
	lgs8gxx_probe_dump(priv, "E-AFTER-RESET");
	lgs8gxx_probe_rb(priv, "E-AFTER-RESET", 0x0C, 0x55);
	lgs8gxx_probe_rb(priv, "E-AFTER-RESET", 0x18, 0x66);
	lgs8gxx_probe_dump(priv, "E-AFTER-RB");
	release_firmware(fw);
	return 0;
}
