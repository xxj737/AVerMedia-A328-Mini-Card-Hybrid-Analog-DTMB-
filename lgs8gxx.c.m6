...
static int lgs8g75_init_data(struct lgs8gxx_state *priv)
{
	...
	/* WINSEQ: soft reset first, then reg 0x18 read-back-rewrite
	 * (from AVerMedia Windows driver sequence analysis) */
	lgs8gxx_soft_reset(priv);
	{
		u8 t18 = 0xff;
		lgs8gxx_read_reg(priv, 0x18, &t18);
		lgs8gxx_write_reg(priv, 0x18, t18);
	}

	lgs8gxx_write_reg(priv, 0xC6, 0x40);
	usleep_range(5000, 10000);	/* wait for download mode */

	lgs8gxx_write_reg(priv, 0x3D, 0x04);
	lgs8gxx_write_reg(priv, 0x3D, 0x04);	/* Windows writes it twice */
	lgs8gxx_write_reg(priv, 0x39, 0x00);
	...
	for (i = 0; i < fw->size; i++) {
		...
		usleep_range(200, 400);	/* per-byte pacing */
	}

	lgs8gxx_write_reg(priv, 0x38, 0x00);

	/* FW-PROBE: after firmware download wait 150ms then read regs that the
	 * 8051 firmware writes early in its startup (0x8012=FF, 0x8013=1F,
	 * 0x8023=01, 0x8022=01, 0x804D=05, 0x804E++).  If any of these I2C
	 * registers mirror the 8051-internal ones, a non-0xFF value proves
	 * the 8051 is actually executing the downloaded firmware. */
	msleep(150);
	{
		u8 rr[16];
		int ri;
		const u8 rlist[] = { 0x00, 0x12, 0x13, 0x22, 0x23,
				     0x4D, 0x4E, 0x48, 0x65, 0x09 };
		for (ri = 0; ri < (int)ARRAY_SIZE(rlist); ri++) {
			lgs8gxx_read_reg(priv, rlist[ri], &rr[ri]);
			printk(KERN_INFO "lgs8gxx: FW-PROBE reg%02X = 0x%02X%s\n",
			       rlist[ri], rr[ri],
			       (rlist[ri] == 0x4E) ? " (expect ~0x05 if 8051 runs)" : "");
		}
		/* second read of 0x4E after 300ms: if it increments, 8051 main
		 * loop (XRAM[0x48]++ -> 0x804E) is definitely alive */
		msleep(300);
		lgs8gxx_read_reg(priv, 0x4E, &rr[0]);
		lgs8gxx_read_reg(priv, 0x48, &rr[1]);
		printk(KERN_INFO "lgs8gxx: FW-PROBE t+300ms reg4E = 0x%02X reg48 = 0x%02X\n",
		       rr[0], rr[1]);
	}
...
static int lgs8gxx_init(struct dvb_frontend *fe)
{
	...
	if (config->prod == LGS8GXX_PROD_LGS8G75) {
		/* mainline downloads firmware at init, not attach */
	...
	/* EXP2-DISABLED 2026-09-01: Windows AVer7231_x64.sys 0x900C8 downloads the
	 * 262-byte firmware then returns WITHOUT any soft reset (no 0x02 write).
	 * Our soft_reset here reset the 8051 back into bootloader (reg0 -> 0x04),
	 * discarding the freshly downloaded firmware. Keep the exact Windows
	 * sequence: download + 0x38=0x00 only. */
	/* lgs8gxx_soft_reset(priv); */
	...
	struct i2c_adapter *i2c)
{
	...
	/* firmware download moved to lgs8gxx_init (mainline behaviour) */
