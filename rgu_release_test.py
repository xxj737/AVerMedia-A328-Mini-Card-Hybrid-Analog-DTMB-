...
print('step0: LGS reg0 before =', hex(rd(0x00)) if rd(0x00) is not None else None)
# ensure RGU released (write again to be sure)
wr('0xE0100','0xFFFFFDD5'); time.sleep(0.2)
print('step1: RGU released, LGS reg0 =', hex(rd(0x00)) if rd(0x00) is not None else None)
print('step2: download fw')
download()
print('  after dl reg0 =', hex(rd(0x00)) if rd(0x00) is not None else None)
srst()
print('step3: after soft_reset reg0 =', hex(rd(0x00)) if rd(0x00) is not None else None)
for r in [0x13,0x34,0x30,0x1f,0x19,0x0c,0x18,0x02]:
    v=rd(r)
