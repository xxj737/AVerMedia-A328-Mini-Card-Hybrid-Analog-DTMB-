...
/*
 * STOCK restore (2026-09-03): send/recv/xfer reverted to the stsp/BeholdT8
 * 2016 reference model (verified shipping on Behold H8 hardware):
 *   - GO bit asserted ONCE per xfer() (in saa7231_i2c_xfer), NOT per byte
 *   - send(): TXBUSY check (5x10ms, auto-hwinit on persistent block) ->
 *             push TX_FIFO -> udelay(200) -> poll TX_FIFO_EMPTY (x1000)
 *   - recv(): poll !RXBUSY (x100) -> read RX_FIFO
 *   - xfer() failure path: plain error return, no implicit engine re-init
 * Rationale: the 9/2 "Route-A" experiment (per_byte_go=1, i2c_poll_us=10,
 * auto-hwinit retry) had NO hardware backing; this file is the clean A/B
 * counterpart.  Keep saa7231_i2c_exit() as void (kernel-5.10 build, header
 * saa7231_i2c.h declares extern void).
 */
