...
int saa7231_msi_init(struct saa7231_dev *saa7231)
{
	...
	dprintk(SAA7231_DEBUG, 1, "MSI Module ID = 0x%08x (POR 0xa1090000)", id);
	if ((id & ~0x00000F00) != MSI_MODULE_ID_POR) {
		/* 芯片可能未完全启动(BSL 未加载)或版本不同；这里不致命,
		 * 继续 probe, 让后续 DVB/I2C 初始化尝试。 */
		dprintk(SAA7231_ERROR, 1, "MSI Module Version 0x%02x, unsupported - continuing", id);
	}
