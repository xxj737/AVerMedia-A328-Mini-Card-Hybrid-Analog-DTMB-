...
#if 0 /* kernel5.10: LINUX_VERSION_CODE needs <linux/version.h> and this legacy
       * pre-4.4 include is unnecessary; INT-A only build below */
...
static int saa7231_enable_msi(struct saa7231_dev *saa7231)
{
	...
	/* kernel5.10: pci_enable_msi() is not usable in this out-of-tree build
	 * (not declared with CONFIG_PCI_MSI off / superseded by irq vectors).
	 * Force INT-A mode which is the default and works fine on A328/BGT. */
	dprintk(SAA7231_ERROR, 1, "MSI not supported on this build, using INT-A");
	return -ENODEV;
}
...
static int saa7231_enable_msix(struct saa7231_dev *saa7231)
{
	...
	dprintk(SAA7231_ERROR, 1, "MSI-X not supported on this build, using INT-A");
	return -ENODEV;
}
...
static void saa7231_free_irq(struct saa7231_dev *saa7231)
{
	...
	/* INT-A only on this build (see saa7231_enable_msi/msix) */
...
int saa7231_pci_init(struct saa7231_dev *saa7231)
{
	...
	/* FIX(kernel5.10): msi_cap must be read BEFORE it is used below;
	 * upstream code read it after computing msi_vectors_max, which used an
	 * uninitialised value and could make kzalloc() below fail with garbage. */
...
MODULE_AUTHOR("Allen Xu <45908351@qq.com>");
MODULE_LICENSE("GPL");
